...
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/basketitems.php"); 

require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/categori_order.php"); 
...
$rsSections->SetUrlTemplates("", $arParams["SECTION_URL"]);
	...
	{
		$rsElement=CIBlockElement::GetList(array(), array("SECTION_ID"=>$arSection["ID"], "IBLOCK_ID"=>$arParams["IBLOCK_ID"]), false, Array("nPageSize"=>50), array("ID", "NAME"));
		while($arElement=$rsElement->GetNext())
		{
			array_push($mqx,$arElement["NAME"]);
		}
     ...
     array(
		...
		while ($arItemz = $dbBasketItems->Fetch()){

			array_push($mass_order,$arItemz["NAME"]);

			if($tdk!=$arItemz["NAME"]){
			$arResult["ELEMENTS"][]=$arItemz;
			array_push($mass_new_order,$arItemz["NAME"]);
			}
			$tdk=$arItemz["NAME"];
		}

		$arSection["ELEMENT_CNT"] = count($mass_order);
		$arSection["NEW_ELEMENT_CNT"]=count($mass_new_order);
